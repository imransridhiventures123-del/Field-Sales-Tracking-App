...
export default function LoginPage() {
  ...
  const validate = () => {
    ...
    if (!/^\d{10}$/.test(mobile)) { setError("Enter a valid 10-digit mobile number."); return false; }
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    try {
      // REAL API CALL — hits POST /api/auth/login on your backend
      const response = await loginUser(mobile, password);
      // response = { token: "eyJ...", user: { name, mobile, employeeId, role, ... } }
