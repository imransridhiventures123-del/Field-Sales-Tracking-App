...
// //  OWNER: Imran
// //  STATUS: Now uses REAL authApi — fake login removed
// // ============================================================
...
// // ↑ These two imports now work because:
// //   AuthContext.jsx  → exists in src/context/
// //   authApi.js       → exists in src/api/
...
//   const { login } = useAuth();
//   // login(token, user) → saves to localStorage + updates global state
...
//       // 2. Save token + user to localStorage and global state
...
//  FILE: src/pages/LoginPage.jsx
//  OWNER: Imran
//  STATUS: DUMMY LOGIN MODE (for frontend development)
//
//  DUMMY CREDENTIALS (share with Naveen for testing):
//    Mobile  : 9876543210
//    Password: admin123
//
//  HOW TO SWITCH TO REAL BACKEND LATER:
//  Search for "DUMMY LOGIN" in this file → delete that block
//  Search for "REAL LOGIN" in this file  → uncomment that block
// ============================================================

import { useState } from "react";
import { useNavigate } from "react-router-dom";
import { useAuth } from "../context/AuthContext";
// import { loginUser } from "../api/authApi"; // ← uncomment when backend ready

// ── DUMMY USER DATA ──────────────────────────────────────────
// This pretends to be what the real backend would return.
// When backend is ready, this whole object gets deleted.
const DUMMY_USERS = [
  {
    mobile: "9876543210",
    password: "admin123",
...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // ════════════════════════════════════════
      //  DUMMY LOGIN — delete this block when
      //  backend is ready
      // ════════════════════════════════════════
      ...
      // ════════════════════════════════════════
      //  REAL LOGIN — uncomment this block
      //  when backend is ready. Delete the
      //  DUMMY LOGIN block above.
      // ════════════════════════════════════════
      // const response = await loginUser(mobile, password);
      // login(response.token, response.user);
      // navigate("/dashboard");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-white flex flex-col items-center justify-center px-4 py-10 relative overflow-hidden">
      ...
      <div className="w-full max-w-sm bg-white rounded-[28px] shadow-xl shadow-blue-100/60 px-8 py-9 relative z-10">
        ...
        {/* Heading */}
        <h1 className="text-[26px] font-bold text-gray-900 mb-1">Welcome Back!</h1>
        <p className="text-sm text-gray-400 mb-4">Login to continue</p>

        {/* ── DUMMY CREDENTIALS HINT BOX ── */}
        {/* Delete this entire box when switching to real backend */}
        <div className="bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 mb-5">
          ...
          <div className="space-y-1">
            <div className="flex justify-between text-xs text-amber-600">
              ...
              <span className="font-mono">9876543210 / admin123</span>
            </div>
            <div className="flex justify-between text-xs text-amber-600">
              <span>Naveen S</span>
              <span className="font-mono">9876543211 / naveen123</span>
            </div>
            <div className="flex justify-between text-xs text-amber-600">
              <span>Imran M</span>
              <span className="font-mono">9876543212 / imran123</span>
            </div>
          </div>
        </div>
        {/* ── END DUMMY HINT BOX ── */}
